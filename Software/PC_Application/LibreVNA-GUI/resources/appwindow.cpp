...
//    qDebug().setVerbosity(0);
    ...
    // Apply UI language before building the UI (so Designer strings are translated)
    {
        QSettings settings;
...
QString AppWindow::LanguageFilePath(const QString &langCode) const
{
    // Expect translation files next to the executable:
    // <app>/i18n/librevna_uk.qm
    const QString base = QCoreApplication::applicationDirPath();
    return QDir(base).filePath(QString("i18n/librevna_%1.qm").arg(langCode));
}

void AppWindow::ApplyLanguage(const QString &langCode)
{
    qApp->removeTranslator(&appTranslator);

    if (langCode == "uk") {
        const QString qmPath = LanguageFilePath("uk");
    ...
    // "en" = default (no translator)
}

void AppWindow::SetupLanguageMenu()
{
    // Insert before Help menu to keep it near the end
    QMenu *menuLanguage = new QMenu(tr("&Language"), this);

    languageActionGroup = new QActionGroup(this);
    languageActionGroup->setExclusive(true);

    QAction *actEn = new QAction(tr("English"), this);
    actEn->setCheckable(true);
    actEn->setData("en");

    QAction *actUk = new QAction(tr("Ukrainian"), this);
    actUk->setCheckable(true);
    actUk->setData("uk");

    languageActionGroup->addAction(actEn);
    languageActionGroup->addAction(actUk);

    menuLanguage->addAction(actEn);
    menuLanguage->addAction(actUk);

    // Put the menu before Help
    ui->menubar->insertMenu(ui->menuHelp->menuAction(), menuLanguage);

    // Restore selection
    QSettings settings;
    const QString lang = settings.value("ui/language", "en").toString();
    if (lang == "uk") actUk->setChecked(true);
    else actEn->setChecked(true);

    connect(languageActionGroup, &QActionGroup::triggered, this, [=](QAction *a) {
        const QString newLang = a->data().toString();
        QSettings s;
        s.setValue("ui/language", newLang);

        ApplyLanguage(newLang);

        // Retranslate Designer UI
        ui->retranslateUi(this);

        // Retranslate our dynamic menu title/actions
        menuLanguage->setTitle(tr("&Language"));
        actEn->setText(tr("English"));
        actUk->setText(tr("Ukrainian"));
    });
}
